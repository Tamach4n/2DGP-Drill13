...
class FixedBackground:
    ...
    def update(self):
        self.window_left = clamp(
            0, int(server.boy.x) - self.cw // 2, self.w - self.cw - 1
        )
        self.window_bottom = clamp(
            0, int(server.boy.y) - self.cw // 2, self.h - self.ch - 1
        )
        pass
...
class NewFixedBackground:
    ...
    def update(self):
        ...
        self.window_bottom = clamp(
            0, int(server.boy.y) - self.ch // 2, self.h - self.ch - 1
        )
...
class TileBackground:
    ...
    def __init__(self):
        ...
        self.tiles = [
            [load_image("cube%d%d.png" % (x, y)) for x in range(3)] for y in range(3)
        ]
...
class InfiniteBackground:
    ...
    def draw(self):
        self.image.clip_draw_to_origin(
            self.q3l, self.q3b, self.q3w, self.q3h, 0, 0
        )  # quadrant 3
        self.image.clip_draw_to_origin(
            self.q2l, self.q2b, self.q2w, self.q2h, 0, self.q3h
        )  # quadrant 2
        self.image.clip_draw_to_origin(
            self.q4l, self.q4b, self.q4w, self.q4h, self.q3w, 0
        )  # quadrant 4
        self.image.clip_draw_to_origin(
            self.q1l, self.q1b, self.q1w, self.q1h, self.q3w, self.q3h
        )  # quadrant 1
